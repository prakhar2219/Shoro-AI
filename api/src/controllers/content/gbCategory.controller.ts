...
export const createGBCategory = async (
  ...
  try {
    const { name, slug, description, content, language_id, order, image, tag, source, author, is_published, created_by, flashcards, mock_test, total_questions, total_time, pass_questions } = req.body;
    ...
    if (slugError) {
      res.status(400).json({ error: `GB Category slug validation failed: ${slugError}. Slug: "${slug}"` });
      return;
    }
...
export const bulkCreateGBCategories = async (
  ...
  try {
    ...
    for (const c of categories) {
      ...
      if (slugError) {
        res.status(400).json({ error: `GB Category slug validation failed: ${slugError}. Slug: "${c.slug}"` });
        return;
      }
