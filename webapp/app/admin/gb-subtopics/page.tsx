...
export default function GBSubtopicsPage() {
  ...
  useEffect(() => {
    getGBTopics().then((topics) => setTopics(Array.isArray(topics) ? topics : []));
  ...
  const gbSubtopicCsvSchema: CsvSchema = useMemo(() => ({
    ...
    description: "Upload a CSV with columns: gb_topic_id, name, slug, description, content, language_id, supported_language_ids (comma-separated IDs - optional), order, image, tag, source, author, is_published, flashcards (true/false - optional), mock_test (true/false - optional), total_questions (number - optional, required if mock_test is true), total_time (number in minutes - optional, required if mock_test is true), pass_questions (number - optional, required if mock_test is true). IMPORTANT: GB Subtopics belong to GB Topics in the General Blogging hierarchy (GB Category → GB Topic → GB Subtopic). Make sure your gb_topic_id corresponds to an existing GB topic. You can find GB topic IDs in the GB Topics admin section.",
    orderConfig: {
  ...
  return (
    ...
    >
      ...
      {/* Global Content Management for All GB Subtopics */}
      <GlobalContentManagement
        entityType="GBSubtopic"
        entityId=""
        entityName="All GB Subtopics"
      />
