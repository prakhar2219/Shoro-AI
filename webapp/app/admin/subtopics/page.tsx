...
export default function SubtopicsPage() {
  ...
  const subtopicCsvSchema: CsvSchema = {
    ...
    description: "Upload a CSV with columns: topic_id, language_id, supported_language_ids (comma-separated IDs - optional), title, slug, author (optional), tag (optional - comma separated), source (optional), content(HTML - optional), order, is_published, flashcards (true/false - optional), mock_test (true/false - optional), total_questions (number - optional, required if mock_test is true), total_time (number in minutes - optional, required if mock_test is true), pass_questions (number - optional, required if mock_test is true). IMPORTANT: Subtopics belong to Topics in the complete educational hierarchy (Board → Class → Subject → Chapter → Topic → Subtopic). Make sure your topic_id and language_id correspond to existing entities in the system.",
    orderConfig: {
