...
export default function GBCategoriesPage() {
  ...
  const gbCategoryCsvSchema: CsvSchema = useMemo(() => ({
    ...
    description: "Upload a CSV with columns: name, slug, description, content, language_id, supported_language_ids (comma-separated IDs - optional), order, image, tag, source, author, is_published, flashcards (true/false - optional), mock_test (true/false - optional), total_questions (number - optional, required if mock_test is true), total_time (number in minutes - optional, required if mock_test is true), pass_questions (number - optional, required if mock_test is true)",
    orderConfig: {
