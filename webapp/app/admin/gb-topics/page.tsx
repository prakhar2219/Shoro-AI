...
export default function GBTopicsPage() {
  ...
  const gbTopicCsvSchema: CsvSchema = useMemo(() => ({
    ...
    description: "Upload a CSV with columns: gb_category_id, name, slug, description, content, language_id, supported_language_ids (comma-separated IDs - optional), order, image, tag, source, author, is_published, flashcards (true/false - optional), mock_test (true/false - optional), total_questions (number - optional, required if mock_test is true), total_time (number in minutes - optional, required if mock_test is true), pass_questions (number - optional, required if mock_test is true). IMPORTANT: GB Topics belong to GB Categories in the General Blogging hierarchy (GB Category → GB Topic). Make sure your gb_category_id corresponds to an existing GB category. You can find GB category IDs in the GB Categories admin section.",
    orderConfig: {
